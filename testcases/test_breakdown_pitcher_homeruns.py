...
analysis_fn, filter_fn, count_fn = analyze_homeruns_gamelog_large_filter_breakdown(analysis_plays)
...
def analyze_pitcher(p: BattingPlay) -> float:
    p_against_breakdown: Dict[str, List[Tuple[float, float]]] = splits_breakdown.get_breakdown_pitcher_pow_against(p.pitching_player.throws)
    pred_strikeout = 0.0
    for bats in p_against_breakdown.keys():
        for rate, avg in p_against_breakdown[bats]:
            p_rating = None
            if p.pitching_player.throws == "L":
                p_rating = p.pitching_player.mov_vl if bats == "L" else p.pitching_player.mov_vr
            else:
                p_rating = p.pitching_player.mov_vr if bats == "R" else p.pitching_player.mov_vl
            pred_strikeout += analysis_fn(p.pitching_player.throws, bats, [avg, p.times_faced_off], [p_rating, p.catcher_player.cabi]) * rate
...
for b in bats:
    for t in throws:
        ...
        tot_diff = sum(map(lambda p: abs(count_fn(p) - analyze_pitcher(p)), flt_b_p_test))
...
tot_diff = sum(map(lambda p: abs(count_fn(p) - analyze_pitcher(p)), filtered_testing_plays))
