...
analysis_fn, filter_fn, count_fn = analyze_strikeouts_gamelog_large_filter_breakdown(analysis_plays)

full_analysis_plays = list(filter(lambda p: p.cur_inning < 7 and p.times_faced_off == 2, analysis_plays))
full_testing_plays = list(filter(lambda p: p.cur_inning < 7 and p.times_faced_off == 2, testing_plays))
...
def analyze_pitcher(p: BattingPlay) -> float:
    p_against_breakdown: Dict[str, List[Tuple[float, float]]] = splits_breakdown.get_breakdown_pitcher_avk_against(p.pitching_player.throws)
    pred_strikeout = 0.0
    for bats in p_against_breakdown.keys():
        for rate, avg in p_against_breakdown[bats]:
            p_rating = None
            if p.pitching_player.throws == "L":
                p_rating = p.pitching_player.stu_vl if bats == "L" else p.pitching_player.stu_vr
            else:
                p_rating = p.pitching_player.stu_vr if bats == "R" else p.pitching_player.stu_vl
