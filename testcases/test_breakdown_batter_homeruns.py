...
def analyze_batter(p: BattingPlay) -> float:
    r_against_breakdown: Dict[str, List[Tuple[float, float]]] = splits_breakdown.get_breakdown_batter_mov_against(p.pitching_player.throws)
    pred_strikeout = 0.0
    for throws in r_against_breakdown.keys():
        for rate, avg in r_against_breakdown[throws]:
            rating = p.batting_player.pow_vl if throws == "L" else p.batting_player.pow_vr
