...
def __get_bat_stat__(p: BattingPlay) -> List[float]:
    if p.pitching_player.throws == "L":
        return [p.batting_player.babip_vl, p.times_faced_off]

    return [p.batting_player.babip_vr, p.times_faced_off]

def __get_pit_stat__(p: BattingPlay) -> List[float]:
    if p.batting_player.bats == "L":
        return [p.pitching_player.gb_type, p.catcher_player.cabi]
...
MIN_CONSTRAINTS = [0, 0, 0, -1, -1]
MAX_CONSTRAINTS = [1, 0, 1, 0, 0]

allowable_hits = ["SINGLE", "DOUBLE", "TRIPLE"]
...
def analyze_hits_gamelog_large_filter(all_plays: List[Play]) -> Tuple[Callable[[BattingPlay], float], Callable[[BattingPlay], bool], Callable[[BattingPlay], float]]:
...
def analyze_hits_gamelog_large_filter_breakdown(all_plays: List[Play]) -> Tuple[Callable[[str, str, float, float], float], Callable[[BattingPlay], bool], Callable[[BattingPlay], float]]:
    batting_plays: List[BattingPlay] = list(filter(lambda p: p.playtype == "BATTING_PLAY", all_plays))
