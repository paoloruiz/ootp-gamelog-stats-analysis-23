...
def __get_bat_stat__(p: BattingPlay) -> List[float]:
    if p.pitching_player.throws == "L":
        return [p.batting_player.avk_vl, p.times_faced_off]

    return [p.batting_player.avk_vr, p.times_faced_off]

def __get_pit_stat__(p: BattingPlay) -> List[float]:
    if p.batting_player.bats == "L":
        return [p.pitching_player.stu_vl, p.catcher_player.cabi]
    elif p.batting_player.bats == "R":
        return [p.pitching_player.stu_vr, p.catcher_player.cabi]
...
MIN_CONSTRAINTS = [-1, -1, 0, 0]
MAX_CONSTRAINTS = [0,  0, 1, 1]

large_filter = ["SAC_BUNT_DP", "SQUEEZE_BUNT", "BUNT_FOR_HIT", "SAC_BUNT_HIT", "SAC_BUNT_OUT", "BUNT_FOR_HIT_OUT", "BUNT_FOR_HIT_DOUBLE_PLAY", "CATCHERS_INTERFERENCE", "BALK", "HIT_BY_PITCH", "WILD_PITCH", "INTENTIONAL_WALK"]
def analyze_strikeouts_gamelog_large_filter(all_plays: List[Play]) -> Tuple[Callable[[BattingPlay], float], Callable[[BattingPlay], bool], Callable[[BattingPlay], float]]:
    ...
    count_fn: Callable[[BattingPlay], float] = lambda p: 1 if "STRIKEOUT" in p.game_sub_result else 0

    potential_so_plays = list(filter(filter_fn, batting_plays))

    so_analysis = batter_v_pitcher_stat_multi_analysis(
        plays=potential_so_plays,
...
def analyze_strikeouts_gamelog_large_filter_breakdown(all_plays: List[Play]) -> Tuple[Callable[[str, str, List[float], List[float]], float], Callable[[BattingPlay], bool], Callable[[BattingPlay], float]]:
    batting_plays: List[BattingPlay] = list(filter(lambda p: p.playtype == "BATTING_PLAY", all_plays))
