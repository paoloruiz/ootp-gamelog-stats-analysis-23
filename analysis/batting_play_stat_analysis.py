...
def attempt_split_high_low(plays: List[BattingPlay], lam_x: Callable[[BattingPlay], float]) -> Tuple[List[BattingPlay], List[BattingPlay]]:
    high_plays = list(filter(lambda p: lam_x(p) >= MID_BREAKPOINT, plays))
    low_plays = list(filter(lambda p: lam_x(p) < MID_BREAKPOINT, plays))
...
def attempt_split_super_high_high_low(plays: List[BattingPlay], lam_x: Callable[[BattingPlay], float]) -> Tuple[List[BattingPlay], List[BattingPlay], List[BattingPlay]]:
    super_high_plays = list(filter(lambda p: lam_x(p) >= HIGH_BREAKPOINT, plays))
    high_plays = list(filter(lambda p: lam_x(p) >= MID_BREAKPOINT and lam_x(p) < HIGH_BREAKPOINT, plays))
    low_plays = list(filter(lambda p: lam_x(p) < MID_BREAKPOINT, plays))
...
def __split_pitcher_stat__(
    ...
    get_pit_stat: Callable[[BattingPlay], float],
    get_analysis: Callable[[List[BattingPlay]], Callable[[float, float], float]],
    ...
    def analyze(b_stat: float, p_stat: float) -> float:
        if p_stat >= HIGH_BREAKPOINT and shp_analysis != None:
            return shp_analysis(b_stat, p_stat)
        elif p_stat >= MID_BREAKPOINT and hp_analysis != None:
...
def __split_batter_stat__(
    plays: List[BattingPlay],
    get_bat_stat: Callable[[BattingPlay], float],
    get_pit_stat: Callable[[BattingPlay], float],
    get_analysis: Callable[[List[BattingPlay]], Callable[[float, float], float]],
    ...
    def analyze(b_stat: float, p_stat: float) -> float:
        if b_stat >= HIGH_BREAKPOINT and shb_analysis != None:
            return shb_analysis(b_stat, p_stat)
        elif b_stat >= MID_BREAKPOINT and hb_analysis != None:
...
def __split_bats__(
    ...
    def analyze(bats: str, b_stat: float, p_stat: float) -> float:
        if bats == "L":
            return lb_analysis(b_stat, p_stat)
        elif bats == "R":
...
def batter_v_pitcher_stat_analysis(
    plays: List[BattingPlay],
    get_bat_stat: Callable[[BattingPlay], float],
    get_pit_stat: Callable[[BattingPlay], float],
    get_analysis: Callable[[List[BattingPlay]], Callable[[int, int], float]],
    attempt_super_split_b: bool = False,
    attempt_super_split_p: bool = False
...
def batter_v_pitcher_stat_analysis_breakdown(
    plays: List[BattingPlay],
    get_bat_stat: Callable[[BattingPlay], float],
    get_pit_stat: Callable[[BattingPlay], float],
    get_analysis: Callable[[List[BattingPlay]], Callable[[int, int], float]],
    attempt_super_split_b: bool = False,
    attempt_super_split_p: bool = False
) -> Callable[[str, str, float, float], float]:
    lp, rp = split_throws(plays)

    lp_analysis = __split_bats__(lp, get_bat_stat=get_bat_stat, get_pit_stat=get_pit_stat, get_analysis=get_analysis, attempt_super_split_b=attempt_super_split_b, attempt_super_split_p=attempt_super_split_p)
    rp_analysis = __split_bats__(rp, get_bat_stat=get_bat_stat, get_pit_stat=get_pit_stat, get_analysis=get_analysis, attempt_super_split_b=attempt_super_split_b, attempt_super_split_p=attempt_super_split_p)

    def analyze(throws: str, bats: str, b_stat: float, p_stat: float) -> float:
        if throws == "L":
